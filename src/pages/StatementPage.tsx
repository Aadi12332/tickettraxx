...
type TabFilter = "All Payments" | "Paid" | "Pending";
...
const TRUCKS = [...new Set(INITIAL_DATA.map((r) => r.truckId))].sort(
  (a, b) => a - b,
);
...
function StatCard({
  ...
  return (
    <div className="bg-white border border-[#E5E7EB] flex items-center rounded-xl px-4 py-4 shadow-[0px_1px_2px_0px_rgba(16,24,40,0.05)]">
      <div className="flex items-center justify-between gap-2 w-full">
        <div className="flex items-center gap-2 min-w-0">
          <div className="w-[40px] h-[40px] rounded-lg bg-[#EAF2FF] border border-[#1D3461] flex items-center justify-center shrink-0">
            {icon}
          </div>

          <div>
            <p className="text-[#667085] text-[12px] font-medium">{label}</p>

            <h2 className="text-[16px] leading-none font-bold text-[#101828] mt-1">
              {value}
            </h2>
          </div>
        </div>
...
function ActionsMenu({ rowId }: { rowId: number }) {
  ...
  const [updatePayModal, setUpdatePayModal] = useState(false);
  ...
  return (
    <>
      ...
      <UpdatePayModal
        open={updatePayModal}
        onClose={() => setUpdatePayModal(false)}
        statementId={selectedRow}
      />
      {showSuccessModal && (
...
export default function StatementPage() {
  ...
  const [tabFilter, setTabFilter] = useState<TabFilter>("All Payments");
  ...
  const filtered = data
    .filter((r) => {
      ...
      const matchTab = tabFilter === "All Payments" || r.status === tabFilter;
  ...
  const statementsAvail = data.length;
  const lastPaymentDate = "02/06/2026";
  ...
  return (
    <div className="bg-[#F3F4F6]">
      ...
      <div className="grid xl:grid-cols-4 sm:grid-cols-2 grid-cols-1 gap-4 mb-5">
        ...
        />
        <StatCard
          icon={<FileText size={18} className="text-[#1D3461]" />}
          label="Statements Available"
          value={String(statementsAvail)}
          trend="+19.01%"
          trendColor="#F97316"
        />
        <StatCard
          icon={<Calendar size={18} className="text-[#1D3461]" />}
        ...
        />
        <StatCard
          icon={<AlertCircle size={18} className="text-[#1D3461]" />}
      ...
      <div className="bg-white rounded-xl border border-[#E5E7EB] px-5 py-4 mb-5 grid xl:grid-cols-6 sm:grid-cols-3 grid-cols-1 gap-4 items-center">
        ...
        <div className="flex flex-col gap-1 flex-1 border border-[#E5E7EB] rounded-lg px-3 py-2.5">
          <label className="text-xs text-[#6B7280]">Select Truck</label>
          <div className="relative">
            <select
              value={truck}
