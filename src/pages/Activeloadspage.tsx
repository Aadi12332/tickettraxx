...
type ActiveLoadsApiResponse = {
  data: ActiveLoadApiItem[];
  pagination?: {
    page: number;
    limit: number;
    total: number;
...
const mapApiLoad = (item: ActiveLoadApiItem, index: number): LoadRow => {
  ...
  return {
    ...
    date: format(safeDate, "MM/dd/yyyy"),
...
function ActionsMenu({ status }: { loadId: number; status: Status }) {
...
function DownloadModal({
  ...
  const downloadExcel = async () => {
    ...
    try {
      const url = new URL("https://65.1.152.16/api/loads/active/export");
      ...
      link.href = href;
      link.download = `active-loads-${format(new Date(), "yyyyMMdd-HHmmss")}.xlsx`;
      document.body.appendChild(link);
      link.click();
      link.remove();
      URL.revokeObjectURL(href);

      setSuccessTitle("Excel");
      setShowSuccess(true);
    } catch (err) {
      ...
      >
        ...
        <div className="p-8 space-y-4">
          ...
          <button
            ...
            disabled={loading}
            className="w-full h-[80px] rounded-lg border border-[#D9D9D9] flex items-center justify-center gap-5 hover:bg-gray-50 transition-colors disabled:cursor-not-allowed disabled:opacity-60"
          >
            <img
...
export default function ActiveLoadsPage() {
  const { token, logout } = useAuth();
  const [data, setData] = useState<LoadRow[]>(INITIAL_DATA);
  ...
  const apiStartDate = appliedFilters.startDate || undefined;
  const apiEndDate = appliedFilters.endDate || undefined;

  const drivers = useMemo(
    () => [...new Set(data.map((r) => r.driverName))],
    [data],
  );

  useEffect(() => {
    setCurrentPage(1);
  }, [showEntries, sortKey, sortDir, appliedFilters, filterStatuses]);
  ...
  useEffect(() => {
    ...
    const loadActiveLoads = async () => {
      ...
      try {
        const url = new URL("https://65.1.152.16/api/loads/active");
        url.searchParams.set("status", "Active");
        if (apiStartDate) url.searchParams.set("startDate", apiStartDate);
        if (apiEndDate) url.searchParams.set("endDate", apiEndDate);
        ...
        const payload = (await response.json().catch(() => null)) as ActiveLoadsApiResponse | null;
        if (response.ok && payload?.data) {
          setData(payload.data.map(mapApiLoad));
        } else {
          setApiError("Unable to load active loads.");
        }
      } catch (error) {
    ...
    void loadActiveLoads();
    return () => controller.abort();
  }, [token, logout, apiStartDate, apiEndDate]);

  const filtered = useMemo(() => {
    return [...data]
      .filter((r) => {
        if (filterStatuses.length > 0 && !filterStatuses.includes(r.status))
          return false;
        if (appliedFilters.driver && r.driverName !== appliedFilters.driver)
          return false;
        return true;
      })
      .sort((a, b) => {
  ...
  }, [data, filterStatuses, appliedFilters, sortKey, sortDir]);
  ...
  return (
    <div className="bg-[#F3F4F6]">
      ...
      <div className="bg-white rounded-xl border border-[#E5E7EB] overflow-hidden">
        <div className="flex flex-col gap-3 px-5 py-3.5 border-b border-[#E5E7EB] sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h2 className="text-base font-semibold text-[#111827]">
              Active Loads
            </h2>
            {apiError ? (
              <p className="text-sm text-[#EF4444] mt-1">{apiError}</p>
            ) : null}
          </div>
